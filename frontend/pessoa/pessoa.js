...
async function buscarPessoa() {
    ...
    const oProfessor = await funcaoEhProfessor(cpf);
    ...
    try {
        const responseAvaliador = await fetch(`${API_BASE_URL}/avaliador/${cpf}`);
        document.getElementById('checkboxAvaliador').checked = responseAvaliador.status === 200;
    } catch (error) {
    ...
    try {
        const responseAvaliado = await fetch(`${API_BASE_URL}/avaliado/${cpf}`);
        document.getElementById('checkboxAvaliado').checked = responseAvaliado.status === 200;
    } catch (error) {
...
async function salvarOperacao() {
    ...
    const pessoa = {
        ...
        EnderecoIdEndereco: formData.get('EnderecoIdEndereco') || null
    };
